/** Changelog:- using access_ok instead of copy_from_user,copy_to_user **/
...
{
	printk("pseudo::entered pseudo read method\n");
	int klen,rcount,ret;
	klen=kfifo_len(&k1);
	if(klen==0)
	{
	...
	kfree(tbuffer);*/
	if(access_ok(VERIFY_WRITE,(void __user*)ubuffer,(unsigned long)count))
	{
		kfifo_out(&k1,ubuffer,rcount);
	}
	else
		return -EFAULT;
	printk("pseudo::read method completed\n");
	return rcount;
}
int pseudo_write(struct file* f1,const char __user* ubuffer,
		size_t ucount,loff_t *off)
{
	...
	//if(ret)
	ret=kfifo_in(&k1,tbuffer,wcount);
	//if(ret)
	kfree(tbuffer);
	**/
	if(access_ok(VERIFY_READ,(void __user*)ubuffer,(unsigned long)count))
	{
		kfifo_in(&k1,ubuffer,wcount);
	}
	else
		return -EFAULT;
	return wcount;
...
static int __init psample_init(void)	
{
	...
	ret=kfifo_alloc(&kfifo,MAX_BUF_SIZE,GFP_KERNEL);
	if(ret)
	{
		//TODO
	}
...
{
	//per device cleanup
	kfifo_free(&kfifo);	//dont deallocate pbuffer again using kfree
	cdev_del(&cdev);
